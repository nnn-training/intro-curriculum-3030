...
const oneTimeTokenMap = new Map(); // キーをユーザ名、値をトークンとする連想配列（サーバーが発行）
...
async function handle(req, res) {
  ...
  switch (req.method) {
    case 'GET':
      ...
      oneTimeTokenMap.set(req.user, oneTimeToken);// サーバーが発行したトークン
    ...
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        const requestedOneTimeToken = params.get('oneTimeToken');// ポストで送られてきたトークン
        if (!content) {
          console.log('content がない');
          handleRedirectPosts(req, res);
          return;
        }
        if (!requestedOneTimeToken) {
          console.log('ワンタイムトークンがない');
          util.handleBadRequest(req, res);
          return;
        }
        if (oneTimeTokenMap.get(req.user) !== requestedOneTimeToken) {
          console.log('サーバーで発行したトークンとPOSTで送られてきたトークンが異なる');
          util.handleBadRequest(req, res);
          return;
        }
